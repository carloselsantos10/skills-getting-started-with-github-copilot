...
def test_signup_and_remove_participant():
    ...
    # Remove
    response_remove = client.delete(f"/activities/{activity}/remove", params={"email": email})
    assert response_remove.status_code == 200
    assert f"Removed {email}" in response_remove.json()["message"]

    # Remove again (should fail)
    response_remove2 = client.delete(f"/activities/{activity}/remove", params={"email": email})
...
def test_remove_activity_not_found():
    response = client.delete("/activities/Nonexistent/remove", params={"email": "a@b.com"})
    assert response.status_code == 404
